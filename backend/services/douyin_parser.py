"""
Douyin video parser – no user cookies required.

Based on public iesdouyin.com API + share-page fallback with WAF solver.
Adapted from https://github.com/Jack-261108/free-video-downloader (MIT-style fork).
"""
...
class DouyinParser:
    ...
    def parse_url(self, url: str) -> VideoInfoResponse:
        ...
        return self._to_video_info(item_info, video_id, resolved_url)
    ...
    def download_to(
        ...
        video_id = self._extract_video_id(resolved_url)
        item_info = self._fetch_item_info(video_id, resolved_url)
        media_url = self._get_media_url(item_info, "video")
        ...
        self._download_file(media_url, filepath, progress_callback)
        return filepath.name, str(filepath.resolve())
    ...
    def _fetch_item_info(self, video_id: str, resolved_url: str) -> dict:
        try:
            ...
            logger.warning("Douyin public API failed (%s), trying share page", e)
    ...
    def _fetch_via_share_page(self, video_id: str, resolved_url: str) -> dict:
        ...
        for node in router_data.get("loaderData", {}).values():
            ...
            if filter_list:
                notice = filter_list[0].get("notice") or filter_list[0].get("detail_msg")
                if notice:
                    raise ValueError(notice)
                reason = filter_list[0].get("filter_reason", "unknown")
                raise ValueError(f"视频不可访问（{reason}），可能已删除或设为私密")
    ...
    def _get_media_url(self, item_info: dict, mode: str = "video") -> str:
        if mode == "video":
            play_urls = (
                item_info.get("video", {}).get("play_addr", {}).get("url_list", [])
            )
            if not play_urls:
    ...
    def _to_video_info(
        self, item_info: dict, video_id: str, webpage_url: str
    ) -> VideoInfoResponse:
        ...
        height = video_info.get("height", 0) or 720

        formats = [
            FormatInfo(
                format_id="douyin_nowm",
                quality=f"无水印 {height}p" if height else "无水印",
                ext="mp4",
            )
        ]
        ...
        return VideoInfoResponse(
            title=title,
            thumbnail=normalize_thumbnail_url(cover_urls[0] if cover_urls else None),
